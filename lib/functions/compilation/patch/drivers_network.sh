...
dirver_rtl8821cs()
{
	# Wireless drivers for Realtek 8821cs chipsets

	if linux-version compare "${version}" ge 5.9 && [ "$EXTRAWIFI" == yes ] ; then

		# attach to specifics tag or branch
		local rtl8821csver="branch:master"

		display_alert "Adding" "Wireless drivers for Realtek 8821cs chipsets ${rtl8821csver}" "info"

		fetch_from_repo "$GITHUB_SOURCE/jack935321350/rtl8821cs" "rtl8821cs" "${rtl8821csver}" "yes"
		cd "$kerneldir" || exit
		rm -rf "$kerneldir/drivers/net/wireless/rtl8821cs"
		mkdir -p "$kerneldir/drivers/net/wireless/rtl8821cs/"
		cp -R "${SRC}/cache/sources/rtl8821cs/${rtl8821csver#*:}"/{core,hal,include,os_dep,platform,halmac.mk,ifcfg-wlan0,rtl8821c.mk,runwpa,wlan0dhcp} \
			"$kerneldir/drivers/net/wireless/rtl8821cs"

		# Makefile
		cp "${SRC}/cache/sources/rtl8821cs/${rtl8821csver#*:}/Makefile" \
			"$kerneldir/drivers/net/wireless/rtl8821cs/Makefile"

		# Kconfig
		sed -i 's/---help---/help/g' "${SRC}/cache/sources/rtl8821cs/${rtl8821csver#*:}/Kconfig"
		cp "${SRC}/cache/sources/rtl8821cs/${rtl8821csver#*:}/Kconfig" \
			"$kerneldir/drivers/net/wireless/rtl8821cs/Kconfig"

		# Adjust path
		sed -i "s/include \$(src)\/rtl8821c.mk/include \$(TopDIR)\/drivers\/net\/wireless\/rtl8821cs\/rtl8821c.mk/" \
			"$kerneldir/drivers/net/wireless/rtl8821cs/Makefile"
		...
		# Add to section Makefile
		echo "obj-\$(CONFIG_RTL8821CS) += rtl8821cs/" >> "$kerneldir/drivers/net/wireless/Makefile"
		sed -i '/source "drivers\/net\/wireless\/ti\/Kconfig"/a source "drivers\/net\/wireless\/rtl8821cs\/Kconfig"' \
			"$kerneldir/drivers/net/wireless/Kconfig"

		# fix compilation for kernels >= 5.4
		process_patch_file "${SRC}/patch/misc/wireless-rtl8821cs-Fix-VFS-import.patch" "applying"
	fi
}
